{ lib, pkgs, ... }:
{
  ...
  programs.zsh.initContent = lib.mkOrder 800 ''
    ...
    # Pure's upstream `prompt_pure_state_setup` captures the user/host segment
    # with a deferred `$prompt_pure_colors[host]` reference. When the prompt is
    # first rendered (especially over SSH, where $ssh_connection triggers the
    # user@host branch), that deferred reference can collapse and the segment
    # renders in the terminal's default foreground instead of the theme hex.
    #
    # Override the function to resolve the color reference eagerly, and also
    # re-run it from `prompt_pure_preprompt_render` after each call to
    # `prompt_pure_set_colors` so theme changes propagate to user/host on
    # every prompt.
